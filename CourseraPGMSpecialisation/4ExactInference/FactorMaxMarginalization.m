...
% till here is just initialisation

% the first is take [1,2,3,4…] and make it into assignments
assignments = IndexToAssignment(1:length(A.val), A.card);
% the assignment of the above look like
% 1 1
% 2 1
% 3 1
% 1 2
...
% so this means that the below will be the same only
% the columns will be mapped to the variables of b
indxB = AssignmentToIndex(assignments(:, mapB), B.card);

% the below will now take the largest elements from A.val(i) and compare if it is
% more than the SINGLE element B.val(indxB(i))
% b has only one variable
% rest are maxed out
