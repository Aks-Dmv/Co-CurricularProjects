...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if(len(OTHER)>=3):
        ...
        for i in range(len(OTHER)-1):
            ...
            sums+=distance_between(OTHER[i+1],OTHER[i])
            theta.append(atan2(OTHER[i+1][1]-OTHER[i][1], OTHER[i+1][0]-OTHER[i][0]))
        disavg= sums/float(len(OTHER)-1)
        ...
        xy_estimate = (measurement[0]+disavg*cos(theta[-1]+avgdtheta), measurement[1]+disavg*sin(theta[-1]+avgdtheta))

    else:
        xy_estimate, OTHER=naive_next_pos(measurement, OTHER)
