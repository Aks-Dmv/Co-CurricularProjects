...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        steering_new=random.gauss(theta,self.steering_noise)
        distance_new=random.gauss(distance,self.distance_noise)

        turn=tan(steering_new)*distance_new/res.length

        if(abs(turn)<0.001):
            res.x=self.x+distance_new*cos(self.orientation)
            res.y=self.y+distance_new*sin(self.orientation)
            res.orientation=(self.orientation+turn)%(2.0*pi)

        else:
            radius=distance_new/turn
