...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    change = tolerance # just for making initial conditions
    while (change>=tolerance):
        change=0.0
        # Change the for loop limits
        for i in range(len(path)): #exclude first and last
            # Very simple change
            # just make sure the prev and next are
            ...
            for j in range(len(path[0])):
                ...
                newpath[i][j] += weight_data * (path[i][j] - newpath[i][j]) + \
                 weight_smooth * (newpath[prev][j] + newpath[succ][j] - 2.0 * newpath[i][j])
